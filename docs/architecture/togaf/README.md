...
This document describes the Workout App architecture by TOGAF domain with a C4-style breakdown inside each domain where helpful.

## Business Architecture

### Business Context
- **Primary actors:** end user (single user on device).
...
- **System:** Workout App (iOS, on-device).
- **External systems:** LLM API (optional), external workout sources (optional), iOS Calendar, HealthKit.
